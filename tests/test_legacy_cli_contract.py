...
PyBreeze starts ``python -m automation_file --execute_str <json>`` (JSON-encoded a second
...
``automation_file.create_project_dir()`` in process. Nothing on the consumer side tests
...
def _actions(target: Path) -> list:
    ...
    return [["FA_create_file", {"file_path": str(target), "content": "x"}]]
...
def test_create_project_dir_is_exported():
    assert callable(automation_file.create_project_dir)
