...
Cross-repo and workspace items live in `D:\Codes\progress.md` (relevant here: X-6, X-7, X-11, X-16, W-5).
...
- **#4** Local housekeeping: four local branches are already merged into `dev` (`codacy-fixes-all-issues`, `feat/new-ops-and-backends`, `fix/action-executor-keys`, `fix/codacy-bandit-test-secret`) and local `main` is behind `origin/main` (workspace W-5).
